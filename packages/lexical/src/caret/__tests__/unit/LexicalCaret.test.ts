  ...
  initializeUnitTest(testEnv => {
    ...
    describe('$removeTextFromCaretRange', () => {
      ...
      describe('select-all + delete collapses to an empty paragraph (#5835)', () => {
        // The collapse lives in RangeSelection.deleteCharacter (the delete key
        // path), not in the shared $removeTextFromCaretRange primitive, so
        // exercise it the way the editor does: select all, then delete. Both
        // directions (Backspace and forward Delete) must collapse.
        function $selectAllAndDelete(isBackward: boolean): void {
          const selection = $selectAll();
          $setSelection(selection);
          selection.deleteCharacter(isBackward);
        }
        for (const isBackward of [true, false]) {
          const label = isBackward ? 'Backspace' : 'Delete';
          test(`heading as the only block collapses (${label})`, () => {
            testEnv.editor.update(
              () => {
                $getRoot()
                  .clear()
                  .append(
                    $createHeadingNode('h1').append($createTextNode('hi')),
                  );
          ...
          test(`quote as the only block collapses (${label})`, () => {
            testEnv.editor.update(
              () => {
                $getRoot()
                  .clear()
          ...
          test(`list followed by a paragraph collapses (${label})`, () => {
            testEnv.editor.update(
              () => {
                $getRoot()
                  .clear()
          ...
          test(`nested list collapses (${label})`, () => {
            testEnv.editor.update(
              () => {
                $getRoot()
                  .clear()
          ...
          test(`paragraph as the first block is left unchanged (${label})`, () => {
            testEnv.editor.update(
              () => {
                const paragraphNode = $createParagraphNode().append(
                  $createTextNode('hi'),
        ...
        test('partial delete that only empties the first block is unaffected', () => {
          testEnv.editor.update(
            () => {
              ...
              $setSelection(sel);
              // Forward delete: a backwards delete of an emptied first block
              // is separately collapsed by the pre-existing collapseAtStart
              // path, which is not what this test is guarding.
        ...
        test('replacing all text in a lone non-paragraph block keeps its type', () => {
          // Guards the Prettier "format" regression: selecting all of a lone
          // block's content and inserting (a replace, which routes through
          // removeText) must keep the block, not collapse it to a paragraph.
          testEnv.editor.update(
            () => {
              const heading = $createHeadingNode('h1').append(
                $createTextNode('hi'),
              ...
              const selection = heading.select(0);
              $setSelection(selection);
              selection.insertText('there');
              const children = $getRoot().getChildren();
              expect(children).toHaveLength(1);
              invariant($isHeadingNode(children[0]), 'Expected a HeadingNode');
              expect(children[0].getTextContent()).toBe('there');
            },
            {discrete: true},
          );
        });
