...
commandWithOpenOptions(
  ...
  .action(function (url, options) {
    codegen(options, url, options.target, options.output).catch(
      logErrorAndExit,
    );
  })
  .addHelpText(
...
_commander.program
  ...
  .action(function (options) {
    (0,
    _gridServer.launchGridServer)(options.agentFactory, options.port || 3333, options.authToken);
  });
...
async function launchContext(options, headless, executablePath) {
  ...
  const contextOptions = options.device // Copy the device descriptor since we have to compare and modify the options.
