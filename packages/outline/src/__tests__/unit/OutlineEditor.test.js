...
describe('OutlineEditor tests', () => {
  ...
  it('Should handle nested updates in the correct sequence', async () => {
    ...
    editor.update(
      () => {
        log.push('A3');
        otlnGetRoot().getLastDescendant().markDirty();
      },
      {
        onUpdate: () => {
          log.push('B3');
  ...
  describe('parseEditorState()', () => {
    ...
    it('Parses the text content of the editor state', async () => {
      expect(parsedEditorState.read(() => otlnGetRoot().__cachedText)).toBe(
        null,
      );
      expect(parsedEditorState.read(() => otlnGetRoot().getTextContent())).toBe(
        'Hello world',
      );
    });
    ...
    it('adds/removes/updates children', async () => {
      async function forPreviousNext(previous: string[], next: string[]) {
        ...
        await update((state: State) => {
          const writableParagraph: ParagraphNode = otlnGetRoot()
            .getFirstChild()
            .getWritable();
