...
export function $insertTableNodesFromClipboard(
  nodes: Array<LexicalNode>,
  selection: BaseSelection,
): boolean {
  const isTableSelection = $isTableSelection(selection);
  const isTableSelectionLike =
    isTableSelection ||
    ('tableKey' in selection && selection.anchor && selection.focus);
  const isRangeSelection = $isRangeSelection(selection);
  const anchorAndFocus = isTableSelectionLike
  ...
  if (
    ...
    (!isTableSelectionLike && !hasOnlyTableNodes)
  ) {
    return false;
  }
  ...
  if (isTableSelectionLike && didPerformMergeOperations) {
...
/** @deprecated renamed to {@link $insertTableNodesFromClipboard} by @lexical/eslint-plugin rules-of-lexical */
export const insertTableNodesFromClipboard = $insertTableNodesFromClipboard;
  ...
  tableObserver.listenersToRemove.add(
    editor.registerCommand(
      ...
      (selectionPayload, dispatchEditor) => {
        ...
        return $insertTableNodesFromClipboard(nodes, selection);
      },
      COMMAND_PRIORITY_HIGH,
    ),
  );
