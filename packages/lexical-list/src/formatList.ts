...
/**
 * @internal
 * Handles Backspace at the start of a list item. If the item is indented,
 * it is outdented one level. If it is at the top level, it is converted
 * to a paragraph (splitting the list if necessary).
 */
export function $handleListBackspaceAtStart(listItem: ListItemNode): boolean {
  if (isNestedListNode(listItem)) {
    return false;
  }
  ...
  if (listItem.getTextContentSize() === 0 && $isListItemNode(previousSibling)) {
    listItem.remove();
