...
`updateFromJSON` and `afterCloneFrom` are all derived from it, and the
`interface` block above narrows their types to this node's serialized shape —
`$config` cannot do that part, because the members have to land on the node's
own interface to override the ones it inherits.
