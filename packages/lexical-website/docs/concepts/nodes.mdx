...
A node's properties have to be described three times: written to JSON, read
back from untrusted JSON, and carried across a clone. `$config`'s `json` option
replaces all three with one declaration of what each property _is_.
...
Their static types stay the ones the node inherits: `exportJSON()` is typed as
`SerializedElementNode` here, whatever it actually writes. That is fine for a
node whose JSON nothing reads back by property name. A node that is part of a
library's public API, whose callers have always read its own serialized type
off `exportJSON()`, narrows the two methods by declaration merging — the one
thing `$config` cannot do, because the members have to land on the node's own
interface to override the ones it inherits:
...
Lexical's own nodes do this so their types stay what they were; a node that
lives inside one application has no such callers and can leave it out.

Stating the domain rather than the type is what earns its keep on the way in.
`updateFromJSON` is the untrusted-JSON boundary: `level: 9` or `level: "big"`
or a missing `level` all land on `1` here, because that is what
`numberValue(1, {integer: true, min: 1, max: 3})` says the property accepts.
Hand-written parsers tend to write `serializedNode.level ?? 1`, which stores
`9` and `"big"` verbatim.

The declaration is also _data_, which several things read:

- **A compact export.** `editorState.toJSON(true)` omits every property whose
  value is the default the parser would restore anyway, because the schema says
  what that default is.
- **Generated code.** Lexical compiles its own nodes' `exportJSON` and
  `updateFromJSON` to straight-line functions from these declarations at build
  time, with each emitted parser checked against the schema it came from.
- **Generated tests.** [`@lexical/fast-check`](/docs/packages/lexical-fast-check)
  turns a schema into a `fast-check` arbitrary, so property-based tests generate
  exactly the JSON the node accepts.
