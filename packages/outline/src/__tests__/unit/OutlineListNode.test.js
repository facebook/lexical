...
describe('OutlineListNode tests', () => {
  initializeUnitTest((testEnv) => {
    test('ListNode.constructor', async () => {
      ...
      await editor.update(() => {
        const listNode = otlnCreateListNode('ul', 1);
      ...
      expect(() => otlnCreateListNode()).toThrow();
    });

    test('ListNode.getTag()', async () => {
      const {editor} = testEnv;
      await editor.update(() => {
        const ulListNode = otlnCreateListNode('ul', 1);
        expect(ulListNode.getTag()).toBe('ul');
        const olListNode = otlnCreateListNode('ol', 1);
    ...
    test('ListNode.canInsertTab()', async () => {
      const {editor} = testEnv;
      await editor.update(() => {
        const listNode = otlnCreateListNode();
    ...
    test('otlnCreateListNode()', async () => {
      const {editor} = testEnv;
      await editor.update(() => {
        const listNode = otlnCreateListNode('ul', 1);
        const createdListNode = otlnCreateListNode('ul');
