...
function getOwnNodeConfig(klass: Klass<LexicalNode>): {
    ...
    if (nodeType) {
      ...
      if (!ownNodeConfig) {
        for (const symbolKey of Object.getOwnPropertySymbols(
          ...
          if (nodeConfigRecord[symbolKey]) {
            ownNodeConfig = nodeConfigRecord[symbolKey];
            break;
          }
        }
      }
...
function compileSetters(klass: Klass<LexicalNode>): readonly CompiledSetter[] {
  ...
  const chain: Array<Klass<LexicalNode>> = [];
  for (
  ...
  for (let i = chain.length - 1; i >= 0; i--) {
    const {ownNodeConfig} = getOwnNodeConfig(chain[i]);
    if (!ownNodeConfig) {
      continue;
    }
...
export function getStaticNodeConfig(klass: Klass<LexicalNode>): {
  ...
  const {ownNodeConfig, ownNodeType} = getOwnNodeConfig(klass);
  if (!isAbstractNodeClass(klass) && ownNodeType) {
