...
- `format` any combination of `bold`, `italic`, `underline`, `strikethrough`, `code`, `highlight`, `subscript`, `superscript`, `lowercase`, `uppercase` and `capitalize`
- `mode`
...
If your node will participate in `@lexical/yjs` collaboration and you store custom data as direct properties instead of
[NodeState](/docs/concepts/node-state), initialize every property unconditionally in the constructor, including `undefined`
defaults. Optional property syntax such as `__foo?: string` without an assignment is not enough, because a freshly
constructed instance will not have that property to sync. See the
[collaboration guide](/docs/collaboration/react#custom-node-property-syncing) for details.
...
## Creating custom nodes with a serialization schema

:::caution Experimental

This feature became available in the lexical v0.51.0 release and is still
considered experimental. Its implementation may change in any release
without a deprecation period.

:::

A node's properties have to be propagated three times: exported to JSON,
imported from untrusted JSON, and carried across a clone.
`$config`'s `json` option replaces all three with a declarative schema.
...
Stating the domain (all possible values the property may inhabit) as data
is the prerequisite that enables all of the new features. In the initial
release, we have:

- **Safer JSON import** - any missing or invalid properties are parsed as
  the default value, instead of potentially silently corrupting the node's
  state
- **Less boilerplate** - all of the boilerplate for `exportJSON`,
  `updateFromJSON` and `afterCloneFrom` is handled automatically. Declaring a
  property as a field says where it is stored, which is exactly what a clone
  has to carry; a property declared through accessor methods names no field, so
  a node with one of those still writes its own `afterCloneFrom`. See
  [Carrying properties across a clone](/docs/serialization/#carrying-properties-across-a-clone).
- **Performance** - a `@lexical/compiler` code generator is used to produce
  optimized implementations of export, import and clone for performance that is
  indistinguishable (or better) than a hand-written implementation.
- **Property Tests** - the `@lexical/fast-check` package can be used to
  generate arbitrary valid serialized nodes for thorough testing of your
  custom nodes. It's never been easier to find edge cases, automatically.

In the future, this declarative schema can be used to assist in supporting
additional protocols, improving developer tools, porting to other
language/platforms, etc.

The combinators (`stringValue`, `numberValue`, `enumValue`, `nullable`,
`optional`, `arrayValue`, `unionValue`, `withField`, `withAccessors`, …) and
the rules for properties that are not plain fields are covered in
[Serialization & Deserialization](/docs/serialization/#declarative-serialization-schemas-with-config).

For performance reasons, you should prefer serialization schema over NodeState
when the property is inherent to the node type and not ad-hoc.

### Backwards compatible return types

That declaration is the whole of the node's serialization. `exportJSON`,
`updateFromJSON` and `afterCloneFrom` are all derived from it.

Their types stay the ones the node inherits: `exportJSON()` is typed as
`SerializedElementNode` here, whatever it actually writes.

Declaration merging can be used to provide a backwards compatible type
interface, for nodes where the `exportJSON()` return type might have been
...
For backwards compatibility, these interfaces are provided for Lexical's
exported node classes. We do not recommend maintaining this type boilerplate
unless you have code that requires it.
...
Using `useReactDecorators`, `PlainTextPlugin` and `RichTextPlugin` executes `React.createPortal(reactDecorator, element)` for each `DecoratorNode`,
...
## Legacy: custom nodes without `$config`

This is how custom nodes were written before `$config` (Lexical v0.33.0). It still works, but is maintained only for backwards compatibility, and the legacy interface may be removed before Lexical 1.0, so prefer the sections above for new code. Every one of these methods is something `$config` writes for you.
