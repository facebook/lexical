...
describe('Regression #TODO', () => {
  class InlineEmptyableElementNode extends ElementNode {
    ...
    ): InlineEmptyableElementNode {
      return new InlineEmptyableElementNode(prevNode.__key);
    }
    static getType() {
      return 'emptyable-inline-element-node';
    }
    static importJSON(serializedNode: SerializedElementNode) {
      return new InlineEmptyableElementNode().updateFromJSON(serializedNode);
    }
    isInline() {
      return true;
    }
  ...
  initializeUnitTest(
    (testEnv) => {
      let inlineElement: InlineEmptyableElementNode;
      beforeEach(() => {
        testEnv.editor.update(
          () => {
            inlineElement = new InlineEmptyableElementNode().append(
            ...
            list.append(
              ...
              $createListItemNode().append(inlineElement),
            );
      ...
      test('Emptyable inline element node removes from emptyable parent ListItem', () => {
        testEnv.editor.update(
          () => {
            inlineElement.clear();
            inlineElement.selectStart();
          },
          {discrete: true},
        );
        testEnv.editor.update(
        ...
        expect(testEnv.container!.innerHTML).toBe(
          `<div contenteditable="true" style="user-select: text; white-space: pre-wrap; word-break: break-word;" data-lexical-editor="true"><ol dir="auto"><li value="1"><span data-lexical-text="true">one</span></li><li value="2"><span data-lexical-text="true">two</span></li></ol></div>`,
        );
      });
    },
    {nodes: [InlineEmptyableElementNode]},
  );
});
