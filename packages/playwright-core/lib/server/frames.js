...
class Frame extends _instrumentation.SdkObject {
  ...
  async _retryWithSelectorIfNotConnected(
    controller,
    selector,
    options,
    action,
    ...
    return controller.run(async (progress) => {
      return this._retryWithProgressIfNotConnected(
        progress,
        selector,
        options.strict,
        (handle) => action(progress, handle),
      );
    }, this._page._timeoutSettings.timeout(options));
  }
  ...
  async focus(metadata, selector, options = {}) {
    const controller = new _progress.ProgressController(metadata, this);
    await this._retryWithSelectorIfNotConnected(
      controller,
      selector,
      options,
      (progress, handle) => handle._focus(progress),
    );
    await this._page._doSlowMo();
  }
  ...
  async expect(metadata, selector, options) {
    ...
    return await this._scheduleRerunnableTaskWithController(
      controller,
      selector,
      (progress, element, options, elements, continuePolling) => {
        if (!element) {
          ...
          const expectsEmptyList =
            ((_options$expectedText = options.expectedText) === null ||
            _options$expectedText === void 0
              ? void 0
              : _options$expectedText.length) === 0;
          if (
            options.expression.endsWith('.array') &&
            expectsEmptyList !== options.isNot
          )
            return {
        ...
        const {matches, received} = progress.injectedScript.expect(
          progress,
          element,
          options,
          elements,
        ...
        (injectedScript, {expression, isFunction, polling, arg}) => {
          ...
          if (typeof polling !== 'number')
            return injectedScript.pollRaf(
              (progress, continuePolling) => predicate(arg) || continuePolling,
            );
