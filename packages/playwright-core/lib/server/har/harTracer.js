...
class HarTracer {
  ...
  async _onRequestFinished(request, response) {
    ...
    const promise = response.body().then(buffer => {
      ...
      if (postData && harEntry.request.postData && this._options.content === 'sha1') {
        harEntry.request.postData._sha1 = (0, _utils.calculateSha1)(postData) + '.' + (_utilsBundle.mime.getExtension(harEntry.request.postData.mimeType) || 'dat');
  ...
  _storeResponseContent(buffer, content) {
    ...
    if (this._options.content === 'embedded') {
      ...
      content._sha1 = (0, _utils.calculateSha1)(buffer) + '.' + (_utilsBundle.mime.getExtension(content.mimeType) || 'dat');
