...
- **Less boilerplate** - all of the boilerplate for `exportJSON`,
  ...
  [Carrying properties across a clone](/docs/serialization/serialization#carrying-properties-across-a-clone).
...
[Serialization & Deserialization](/docs/serialization/serialization#declarative-serialization-schemas-with-config).
...
This is how custom nodes were written before `$config` (Lexical v0.33.0). It still works, and nothing here is deprecated — but every one of these methods is something `$config` writes for you, so prefer the sections above for new code.
